...
class _VideoPlayerScreenState extends BaseWidgetState<VideoPlayerScreen> {
  ...
  Widget buildBody(BuildContext context) {
    return AspectRatio(aspectRatio: 16 / 9, child: videoPlayer());
  }
    ...
    _chewieController = ChewieController(
      ...
      additionalOptions: (context) {
        return <OptionItem>[
          ...
          OptionItem(
            onTap: (_) => debugPrint('Another option working!'),
            iconData: Icons.chat,
      ...
      errorBuilder: (context, errorMessage) {
        return Center(child: FrdText(errorMessage));
      },
    );
  }
  ...
  Future<ClosedCaptionFile> _loadCaption() async {
    final String fileContents = await DefaultAssetBundle.of(
      context,
    ).loadString('assets/captions/bumble_bee_captions.vtt');
